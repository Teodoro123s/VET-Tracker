    ...
    try {
      ...
      if (!superAdminSnapshot.empty) {
        ...
        if (superAdminData.password !== password) {
          return { success, error: 'Incorrect password. Please try again.' };
        }
      ...
      if (querySnapshot.empty) {
        return { success, error: 'No account found with this email address' };
      }
      ...
      if (userData.status === 'inactive' || userData.status === 'suspended') {
        return { success, error: 'This account has been disabled. Contact support.' };
      }
      ...
      return { success, error: 'Login failed. Please check your internet connection and try again.' };
    }
  };
