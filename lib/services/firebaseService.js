...
const getTenantId = async (userEmail) => {
  ...
  try {
    ...
    // Fallback email prefix as tenant ID
...
export const getSubscriptionData = async () => {
  try {
    const tenants = await getAllTenants();
    const statusCounts = { active, pending, expired, trial};
...
export const getCustomerById = async (userEmail?, customerId?) => {
  try {
    const tenantId = await getTenantId(userEmail || '');
...
export const getMedicalFormById = async (userEmail?, formId?) => {
  try {
    const tenantId = await getTenantId(userEmail || '');
    if (!tenantId || !formId) {
      return null;
...
export const getPetById = async (userEmail?, petId?) => {
  try {
    const tenantId = await getTenantId(userEmail || '');
    if (!tenantId || !petId) {
      return null;
...
export const updatePet = async (petId, updateData, userEmail?) => {
  try {
    const tenantId = await getTenantId(userEmail || '');
...
export const getMedicalHistory = async (userEmail?, petId?) => {
  try {
    const tenantCollection = await getTenantCollection(userEmail || '', 'medicalRecords');
    const querySnapshot = await getDocs(tenantCollection);
...
export const createTenant = async (tenantId, clinicData) => {
  try {
    ...
    for (const col of collections) {
      await addDoc(collection(db, `tenants/${tenantId}/${col}`), { _init});
    }
  } catch (error) {
...
export const deleteCustomerWithPets = async (customerId, userEmail?) => {
  try {
...
export const deletePet = async (id, userEmail?) => {
  try {
...
export const deleteVeterinarian = async (id, userEmail?) => {
  try {
...
export const deleteMedicalRecord = async (id, userEmail?) => {
  try {
    const tenantId = await getTenantId(userEmail || '');
...
export const deleteMedicalCategory = async (id, userEmail?) => {
  try {
    const tenantId = await getTenantId(userEmail || '');
...
export const deleteMedicalForm = async (id, userEmail?) => {
  try {
    const tenantId = await getTenantId(userEmail || '');
...
export const addMedicalForm = async (formData, userEmail?) => {
  try {
    const tenantCollection = await getTenantCollection(userEmail || '', 'medicalForms');
...
export const addMedicalCategory = async (categoryData, userEmail?) => {
  try {
    const tenantCollection = await getTenantCollection(userEmail || '', 'medicalCategories');
...
export const addFormField = async (fieldData, userEmail?) => {
  try {
    const tenantCollection = await getTenantCollection(userEmail || '', 'formFields');
...
export const updateMedicalForm = async (formId, updateData, userEmail?) => {
  try {
    const tenantId = await getTenantId(userEmail || '');
    const collectionPath = tenantId ? `tenants/${tenantId}/medicalForms` : 'medicalForms';
...
export const deleteFormField = async (fieldId, userEmail?) => {
  try {
    const tenantId = await getTenantId(userEmail || '');
...
export const updateFormField = async (fieldId, fieldData, userEmail?) => {
  try {
    const tenantId = await getTenantId(userEmail || '');
    const collectionPath = tenantId ? `tenants/${tenantId}/formFields` : 'formFields';
...
export const getStaff = async (userEmail?) => {
  try {
...
export const addStaff = async (staffData, userEmail?) => {
  try {
    const tenantCollection = await getTenantCollection(userEmail || '', 'staff');
...
export const updateStaff = async (staffId, updateData, userEmail?) => {
  try {
    const tenantId = await getTenantId(userEmail || '');
...
export const deleteStaff = async (staffId, userEmail?) => {
  try {
    const tenantId = await getTenantId(userEmail || '');
    const collectionPath = tenantId ? `tenants/${tenantId}/staff` : 'staff';
...
export const getAnimalTypes = async (userEmail?) => {
  try {
...
export const addAnimalType = async (animalTypeData, userEmail?) => {
  try {
    const tenantCollection = await getTenantCollection(userEmail || '', 'animalTypes');
...
export const updateAnimalType = async (animalTypeId, updateData, userEmail?) => {
  try {
    const tenantId = await getTenantId(userEmail || '');
...
// Breed functions
export const getBreeds = async (userEmail?) => {
  try {
...
export const addBreed = async (breedData, userEmail?) => {
  try {
    const tenantCollection = await getTenantCollection(userEmail || '', 'breeds');
...
export const deleteAnimalType = async (animalTypeId, userEmail?) => {
  try {
    const tenantId = await getTenantId(userEmail || '');
    const collectionPath = tenantId ? `tenants/${tenantId}/animalTypes` : 'animalTypes';
...
export const deleteBreed = async (breedId, userEmail?) => {
  try {
    const tenantId = await getTenantId(userEmail || '');
...
export const generatePassword = async (targetEmail, initiatorEmail, initiatorType) => {
  ...
  await addDoc(collection(db, collectionPath), {
    email,
    pendingPassword,
    pendingPasswordCreatedAt Date().toISOString(),
    pendingPasswordExpiresAt Date(Date.now() + 24*60*60*1000).toISOString(),
  ...
  return { success, message: 'New password sent to email' };
};

export const generateOwnPassword = async (vetEmail) => {
...
export const loginWithCredentialOverlap = async (email, password) => {
  try {
    const tenantId = await getTenantId(email || '');
    const collectionPath = tenantId ? `tenants/${tenantId}/userCredentials` : 'userCredentials';
    ...
    if (latestCred.currentPassword && await verifyPassword(password, latestCred.currentPassword)) {
      return { success, user: { email: latestCred.email } };
    }
    
    // Check pending password if exists and not expired
    if (latestCred.pendingPassword && new Date() {
  try {
...
export const getTransactionSummary = async (timeFilter) => {
  try {
    ...
    return {
      totalRevenue,
      monthlyRevenue / 12,
      averageTransaction > 0 ? totalRevenue / transactionCount,
      transactionCount,
      revenueGrowth: 12.5
    };
  } catch (error) {
...
export const getFinancialData = async (timeFilter) => {
  try {
    const snapshot = await getDocs(collection(db, 'transactions'));
