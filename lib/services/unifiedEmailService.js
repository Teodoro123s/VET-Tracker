...
// Send email using EmailJS (removed)
const sendWithEmailJS = async (emailData) => {
  console.warn('sendWithEmailJS: EmailJS removed; falling back to Resend/logging.', emailData);
  // As EmailJS has been removed, emulate a successful send so callers don't break.
  return { ok: true, provider: 'disabled-emailjs', info: { to: emailData.to } };
};
