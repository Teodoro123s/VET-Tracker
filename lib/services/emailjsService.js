...
// Veterinarian-specific EmailJS config
const EMAILJS_VET_SERVICE_ID = extra.EMAILJS_VET_SERVICE_ID || process.env.EMAILJS_VET_SERVICE_ID || 'service_zefpsar';
const EMAILJS_VET_TEMPLATE_ID = extra.EMAILJS_VET_TEMPLATE_ID || process.env.EMAILJS_VET_TEMPLATE_ID || 'template_hoxe5k7';
const EMAILJS_VET_PUBLIC_KEY = extra.EMAILJS_VET_PUBLIC_KEY || process.env.EMAILJS_VET_PUBLIC_KEY || 'ReUQK_p4eYlnZileo';

// Debug: Log veterinarian EmailJS config on load
console.log('Veterinarian EmailJS Config:', {
  serviceId: EMAILJS_VET_SERVICE_ID,
  templateId: EMAILJS_VET_TEMPLATE_ID,
  publicKey: EMAILJS_VET_PUBLIC_KEY,
  extraValues: {
    serviceId: extra.EMAILJS_VET_SERVICE_ID,
    templateId: extra.EMAILJS_VET_TEMPLATE_ID,
    publicKey: extra.EMAILJS_VET_PUBLIC_KEY
  }
});
    ...
    const response = await fetch('https://api.emailjs.com/api/v1.0/email/send', {
      ...
      body: JSON.stringify({
        ...
        template_params: {
          ...
          portal_url: 'https://your-portal-url.com' // Update with your actual portal URL
        }
      })
    });
...
// Send veterinarian credentials email
exports.sendVeterinarianCredentialsEmail = async (to, name, email, password) => {
  if (!EMAILJS_VET_SERVICE_ID || !EMAILJS_VET_TEMPLATE_ID || !EMAILJS_VET_PUBLIC_KEY) {
    console.warn('sendVeterinarianCredentialsEmail: Veterinarian EmailJS not configured; no email sent.', { to, email });
    return { success: false, message: 'Veterinarian EmailJS not configured.' };
  }

  console.log('Sending veterinarian email with:', {
    service_id: EMAILJS_VET_SERVICE_ID,
    template_id: EMAILJS_VET_TEMPLATE_ID,
    user_id: EMAILJS_VET_PUBLIC_KEY,
    to_email: to,
    to_name: name
  });
  ...
  try {
    ...
    if (response.ok) {
      console.log('EmailJS veterinarian credentials email sent successfully to', to);
      return { success: true };
    } else {
      throw new Error(`EmailJS failed: ${response.status} ${responseText}`);
    }
  } catch (error) {
    console.error('sendVeterinarianCredentialsEmail error:', error);
    return { success: false, error: error.message };
  }
};
